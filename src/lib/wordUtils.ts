...
export interface MultiSectionWordOptions {
  title: string;
  subtitle?: string;
  sections: {
    title: string;
...
export const generateMultiSectionWord = async ({
  title: rawTitle,
  subtitle: rawSubtitle,
  sections,
  fileName
}: MultiSectionWordOptions) => {
  ...
  for (const sec of sections) {
    ...
    const tableHeader = new TableRow({
      children: sec.columns.map(col => new TableCell({
    ...
    const tableRows = sec.data.map((row, rowIndex) => {
      const firstCellRaw = String(row[0] || '').trim().toUpperCase();
      const isSubtotalOrTotal = firstCellRaw.includes('TOTAL') || firstCellRaw.includes('SUBTOTAL') || firstCellRaw.includes('CONSOLIDADO') || firstCellRaw.includes('SOMA');
            ...
            return new TableCell({
              children: [new Paragraph({
                children: [new TextRun({ 
                  text: cellTextRaw, 
                  bold: true, 
                  color: firstCellRaw.includes('SUBTOTAL') ? '064E3B' : 'FFFFFF',
                  size: 16
                })],
                alignment: colIndex === 0 ? AlignmentType.LEFT : AlignmentType.CENTER
              })],
              shading: { fill: firstCellRaw.includes('SUBTOTAL') ? 'D1FAE5' : '10b981' }
            });
          }
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          children: [
            new TextRun({ text: 'CNPJ: 10.706.425/0001-74 • MA-014, Alto São Francisco, Vitória do Mearim – Maranhão', size: 15, color: '666666', bold: true })
          ],
          alignment: AlignmentType.CENTER,
          spacing: { after: 120 }
        }),
        new Paragraph({
          children: [new TextRun({ text: 'Relatório Oficial de Almoxarifado e Estoque', size: 18, color: '888888', italics: true })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 150 }
        }),
        new Paragraph({
        ...
        new Paragraph({
          children: [
            new TextRun({ text: 'Assinatura do Profissional Responsável', bold: true, size: 18, color: '333333' })
          ],
          alignment: AlignmentType.CENTER,
          spacing: { after: 300 }
        })
